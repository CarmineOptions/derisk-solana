...
COLLECT_INTERVAL_SECONDS: int = 5 * 60 # Collect and store orderbook liquidity every 5 minutes
...
async def update_ob_dex_data_continuously():
	...
	while True:
		...
		time.sleep(
			max(0, COLLECT_INTERVAL_SECONDS - execution_time)
		)
...
def load_ob_dex_data(start_timestamp: int) -> list[db.CLOBLiqudity]:
	...
	with db.get_db_session() as session:
		entries = session.query(
			...
			db.CLOBLiqudity.timestamp >= start_timestamp
		).all()
