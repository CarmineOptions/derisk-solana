...
# Load your IDL file
with open('../marginfi_idl2.json', 'r') as fp:
...
async def fetch_and_parse_transactions():
    ...
    res = await solana_client.get_signatures_for_address(
        program_id,  # Address
        limit=5,
        before = Signature.from_string("4if5hzfKbJSxbA13ReBca2sY6kbAias7FseHG1QdUCHqSjhG4eNbGoz96TbBSNmCGoSQfKWx5gMtDVgxNb1iUV2M")
    )
    signatures = [i.signature for i in res.value]
    signatures = [Signature.from_string("4if5hzfKbJSxbA13ReBca2sY6kbAias7FseHG1QdUCHqSjhG4eNbGoz96TbBSNmCGoSQfKWx5gMtDVgxNb1iUV2M")]
